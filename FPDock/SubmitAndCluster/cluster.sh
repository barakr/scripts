...
set apc_column=`head -1 $scorefile | awk '{for (i=1;i<=NF;i++){if ($i=="atom_pair_constraint") print i}}'`
...
#if ( ! -e clog ) then
$PATH_TO_EXE/cluster.default.linuxgccrelease -in:file:silent $decoys -in:file:silent_struct_type binary -database $PATH_TO_DB -cluster:radius $actualR -in:file:fullatom -tags `cat $scorefile | sort -nrk ${stype_column} | tail -$top | awk '{print $'"${description_column}"'}'` -silent_read_through_errors >! clog
#endif
...
cat $scorefile | awk '{print $'"$description_column"',$'"$stype_column"',$'"$rmsBB_if_column"',$'"$bestRMS_5mer_all_column"',$'"$startRMSbb_column"',$'"$startRMSphipsi_column"',$'"$apc_column"'}' | sort > tmp.listB
