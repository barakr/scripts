...
# e.g.
# $ cluster_shuf2000.sh 500 2 ../score_flags_abinitio.1.sc ../run_src/native.pdb ../decoys_flags_abinitio.silent reweighted_sc
...
if (! -e c.0.0.pdb ) then
	$PATH_TO_EXE/cluster.linuxgccrelease -in:file:silent $decoys -in:file:silent_struct_type binary -database $PATH_TO_DB -cluster:radius $actualR -in:file:fullatom -tags `shuf $scorefile | head -n 2000 | sort -nrk ${stype_column} | tail -$top | awk '{print $'"${description_column}"'}'` -silent_read_through_errors >! clog
endif
